...
\begin{table}[h]
	...
	\label{uitreksel}
	% TODO: Append tables for and bedrooms
\end{table}
...
\end{table}
	...
	\begin{table}[h]
		...
		naïefmodel & $0.096$& \textit{realSum} $\sim$ \textit{attr}$+$\textit{satisfaction}$+$\textit{dist}\\
		 logattrmodel &$0.096$ & \textit{realSum} $\sim$ $log_{10}$(\textit{attr})$+$\textit{satisfaction}$+$\textit{rest}$+$\textit{dist}\\
		 logmodel &$0.19$ & $log_{10}$(\textit{realSum}) $\sim$ \textit{attr}$+$\textit{satisfaction}$+$\textit{metro}$+$\textit{dist}\\
		 logmodel2 &$0.19$ & $log_{10}$(\textit{realSum}) $\sim$ \textit{attr}$+$\textit{satisfaction}$+$\textit{dist}\\
	...
	\end{equation} % TODO: Herschrijf per de normen in feedback 18-19
